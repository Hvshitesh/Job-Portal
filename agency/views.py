...
def client_list(request):
    ...
    if request.method == "POST":
        if request.POST["search_text"]:
            clients = clients.filter(
                Q(name__icontains=request.POST["search_text"])
                | Q(website__icontains=request.POST["search_text"])
            )
    ...
    return render(
        request,
        "recruiter/company/client_list.html",
        {
            "clients": clients,
            "aft_page": aft_page,
...
def add_client(request):
    if request.user.is_agency_admin:
        if request.method == "POST":
            validate_client = ClientForm(request.POST, request.FILES, user=request.user)
            errors = client_errors(validate_client.errors, request.POST)
            if not errors:
                client = AgencyCompany.objects.create(
                ...
                for each in range(1, int(request.POST.get("no_of_branch")) + 1):
                    if request.POST.get("location_" + str(each)):
                        ...
                        is_major = request.POST.get("is_major_" + str(each), "")
        ...
        return render(
            request,
            "recruiter/company/add_client.html",
            {"cities": cities, "months": MONTHS},
        )
    else:
        message = "Sorry, You Don't have access to this page"
        return render(
            request, "recruiter/recruiter_404.html", {"message": message}, status=404
        )
    ...
    if request.user.is_agency_admin:
        ...
        if client:
            if request.method == "GET":
                ...
                return render(
                    request,
                    "recruiter/company/edit_client.html",
                    {"client": client, "cities": cities, "months": MONTHS},
                )
            ...
            if not errors:
                ...
                for each in range(1, int(request.POST.get("no_of_branch")) + 1):
                    address = request.POST.get("address_" + str(each))
                    contact_details = request.POST.get("contact_details_" + str(each))
                    is_major = request.POST.get("is_major_" + str(each), "")
                    if (
                        request.POST.get("location_" + str(each))
                        and address
                        and contact_details
                    ):
                        city = City.objects.get(
...
def delete_client(request, client_id):
    ...
    if user.is_agency_admin:
        client = AgencyCompany.objects.filter(id=client_id, company=user.company)
        if client:
            client = client[0]
            client.delete()
        ...
        data = {
            "error": True,
            "response": "Only Company Admin can delete their clients",
        }
    return HttpResponse(json.dumps(data))
...
def client_profile(request, client_id):
    client = AgencyCompany.objects.filter(id=client_id, company=request.user.company)
    if client:
        client = client[0]
...
def add_branch(request, branch_id):
    ...
    return render(
        request,
        "recruiter/company/add_branch.html",
        {"branch_id": branch_id, "cities": cities},
    )
...
def add_contract_deatils(request, contact_details_id):
    ...
    return render(
        request,
        "recruiter/company/add_contract_details.html",
        {"months": MONTHS, "contact_details_id": contact_details_id},
    )
...
def dashboard(request):
    if request.user.is_agency_admin:
        ...
        job_posts = JobPost.objects.filter(agency_recruiters__in=[request.user])
    if request.user.is_agency_admin:
        agency_resumes = AgencyResume.objects.filter(
    ...
    return render(
        request,
        "recruiter/company/dashboard.html",
        {
...
def applicant_status_change(request, applicant_id):
    ...
    if request.POST:
        ...
        applicant.save()
    return HttpResponse(
        json.dumps({"error": False, "response": "Status Changed Successfully"})
    )
...
def view_resumes(request, job_post_id):
    ...
    return render(
        request,
        "recruiter/company/job_resume_view.html",
        {
            "job_post": job_post,
            "agency_resumes": agency_resumes,
...
def job_status_change(request, job_post_id):
    ...
    if agency_jobposts:
        ...
        return HttpResponseRedirect(
            reverse("agency:view", kwargs={"job_post_id": job_post_id})
        )
...
def jobs_billing_process(request, job_post_id):
    ...
    if request.user.is_agency_admin:
        ...
        agreed_percantage_amount = (
            float(total_amount) * float(agreed_percantage)
        ) / 100.0
        ...
        agreed_percantage_amount = float_round(agreed_percantage_amount, 2, ceil)

        deducted_ammount = (
            float(service_tax)
            + float(swatch_barath_cess)
            + float(krushi_kalyani_cess)
            + float(agreed_percantage_amount)
        )
        total_invoice_amount = float(total_amount) - deducted_ammount

        # import pdfkit
        data = {
            "agreed_percantage": agreed_percantage,
            "total_amount": total_amount,
            "service_tax": service_tax,
            "swatch_barath_cess": swatch_barath_cess,
            "krushi_kalyani_cess": krushi_kalyani_cess,
            "agreed_percantage_amount": agreed_percantage_amount,
            "deducted_ammount": deducted_ammount,
...
def hired_candidates(request, job_post_id):
    ...
    return render(
        request,
        "recruiter/company/jobs_hired_resumes.html",
        {
            "agency_resumes": agency_resumes,
            "recruiters": recruiters,
            "years": years,
...
def user_work_log(request):
    ...
    if validate_worklog.is_valid():
        start_time = datetime.strptime(
            request.POST.get("start_time"), "%m/%d/%Y %H:%M:%S"
        ).strftime("%Y-%m-%d %H:%M:%S")
        end_time = datetime.strptime(
            request.POST.get("end_time"), "%m/%d/%Y %H:%M:%S"
        ).strftime("%Y-%m-%d %H:%M:%S")

        AgencyWorkLog.objects.create(
            job_post=job_post,
            user=request.user,
